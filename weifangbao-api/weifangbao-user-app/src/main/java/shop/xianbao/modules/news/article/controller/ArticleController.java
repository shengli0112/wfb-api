...
public class ArticleController {
    ...
    @Value("${app_url.news_url}")
    String configNewsUrl;
    ...
    @GetMapping("/scrollingScreenList")
    @ApiOperation("滚屏公告列表")
    @ApiImplicitParams({
        @ApiImplicitParam(name = Constant.PAGE, value = "当前页码，从1开始", paramType = "query", required = true, dataType = "int"),
        @ApiImplicitParam(name = Constant.LIMIT, value = "每页显示记录数", paramType = "query", required = true, dataType = "int")
    ...
    public Result<List<ScrollingScreenDTO>> scrollingScreenList(@ApiIgnore @RequestParam Map<String, Object> params) {
        List<ScrollingScreenDTO> page = articleService.getscrollingScreenList(params, configNewsUrl);
        return new Result<List<ScrollingScreenDTO>>().ok(page);
    }
