package shop.xianbao.modules.pay.controller.wx;
...
public class WxPayController {
    ...
    public Result sendMessage() {
        String openId = "otMZTwjCM8q8UPe6FMwzVECTdZBo";
        String content = "您购买的商品已发货";
        String orderSn = "12345678";
        wxService.sendWxDeliverMessage(openId, content, orderSn, new Date(), "备注");
        return null;
    }

    @Login
    @GetMapping("/getOpenId")
    public Result getOpenId(@RequestParam(name = "code") String code, @LoginUser MemberDataEntity user) {
        if (StringUtils.isBlank(code)) {
            return new Result().error("code不存在");
        }
        try {
            ...
            if (StringUtils.isBlank(wxMpUser.getOpenId())) {
                return new Result().error("用户数据不存在");
            }
            //保存open_id到union_user
            UnionUserEntity userEntity = new UnionUserEntity();
            userEntity.setId(user.getUnionId());
            userEntity.setOpenId(wxMpUser.getOpenId());
            unionUserService.updateById(userEntity);
            return new Result().ok(wxMpUser);
        } catch (WxErrorException e) {
            e.printStackTrace();
            return new Result().error("授权登录失败");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * <pre>
     * 查询订单(详见https://config.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_2)
    ...
    public WxPayOrderQueryResult queryOrder(
        @RequestParam(required = false) String transactionId, @RequestParam(required = false) String outTradeNo) throws WxPayException {
    ...
    public WxPayOrderQueryResult queryOrder(WxPayOrderQueryRequest wxPayOrderQueryRequest) throws WxPayException {
        return this.wxPayService.queryOrder(wxPayOrderQueryRequest);
    }
    ...
    public WxPayOrderCloseResult closeOrder(WxPayOrderCloseRequest wxPayOrderCloseRequest) throws WxPayException {
        return this.wxPayService.closeOrder(wxPayOrderCloseRequest);
    }
    ...
    public Result createOrder(@RequestBody WxPayUnifiedOrderRequest request, @LoginUser MemberDataEntity user) throws WxPayException {
        ...
        request.setNotifyUrl("https://ejr9bj.natappfree.cc/xianbao_user_app/wxpay/notify/order");
        return this.wxPayService.createOrder(request);
    }

    /**
     * 统一下单(详见https://config.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_1)
    ...
    public WxPayRefundQueryResult refundQuery(WxPayRefundQueryRequest wxPayRefundQueryRequest) throws WxPayException {
        return this.wxPayService.refundQuery(wxPayRefundQueryRequest);
    }
    ...
    public WxPayBillResult downloadBill(WxPayDownloadBillRequest wxPayDownloadBillRequest) throws WxPayException {
        return this.wxPayService.downloadBill(wxPayDownloadBillRequest);
    }
    ...
    public String queryComment(Date beginDate, Date endDate, Integer offset, Integer limit) throws WxPayException {
        return this.wxPayService.queryComment(beginDate, endDate, offset, limit);
    }
